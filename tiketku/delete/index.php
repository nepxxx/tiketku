...
/* by @anandabaskara from Visual Studio Code with ❤️ */
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        try {
            ...
            //start query delete
            $idtiket = sanitasiVariabel($dbConn, $_POST['idtiket']);

            $SQL = "SELECT * FROM tb_tiket WHERE id_tiket = '$idtiket'";
            $result = mysqli_query($dbConn, $SQL);

            //check if the id is not registered or if the connection is not estabilished
            if (mysqli_num_rows($result) > 0) {
            $SQL = "DELETE FROM tb_tiket WHERE id_tiket = '$idtiket'";
            if (!(mysqli_query($dbConn, $SQL))) {
                throw new \Exception("Error Processing Request, Failed to Delete the Data", 1);
            }
            } else {
                throw new \Exception("Error Processing Request, ID: $idtiket Not Found", 1);
            }
            //end-delete
            ...
            $response = array(
                'status' => true,
                'informasi' => "Data has been deleted",
            );
